...
{
    Console.WriteLine("[提示] 未找到共享内存 Local\\ETS2NavTelemetry。");
    Console.WriteLine("请确认：① scs-nav-bridge.dll 已复制到游戏 bin\\win_x64\\plugins\\；");
    Console.WriteLine("② 游戏已启动并进入驾驶界面（插件在游戏加载插件时创建共享内存）。");

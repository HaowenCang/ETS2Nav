...
| ID | 内容 | 前置 | 状态 | 验证/产出 |
|---|---|---|---|---|
| A1 | P3 提醒模块核心逻辑 | — | ⏳ | 合成回放断言 + Europe 全量构建；启用边界待 B2 |
| A2 | P4 UI（Browser/Desktop/LAN Mobile） | A1 事件接口（可先以合成事件） | ⏳ | 回放驱动交互验证；60 FPS 帧率 |
| A3 | P5 OD corpus 与全欧洲检查 | — | ⏳ | run-p5-tests.bat 全绿 |
| A4 | P6 性能优化（ALT/CH 评估、增量编译） | — | ⏳ | synthetic benchmark；B6 为最终依据 |
| A5 | B7 工具准备 | — | ⏳ | signal-lab 扩展就绪 |
| B1 | 插件安装与冒烟 | — | ⏳ | nav-core-cli live 输出持续帧 |
| B2 | 单趟综合驾驶采集（T1–T6） | B1 | ⏳ | trace + FPS 记录；关闭 P2 已知限制 |
| B3 | B7 特殊行为实验 | A5 | ⏳ | warp/reset/特殊 profile 行为判定 |
| B4 | P3 提醒实机验收 | A1、B2 | ⏳ | 一致率/误报/播报体验记录 |
| B5 | P4 移动端实测 | A2 | ⏳ | 扫码连接 + 60 FPS + 断线重连 |
| B6 | §63 正式性能验收 | A2/A4 完成态 | ⏳ | PresentMon 对比数据达标 |

---

## §6 建议节奏

1. **即日起（A 侧连续推进）**：A1 限速链与提醒模块 → A2 UI（并行）→ A3/A4/A5。
2. **用户侧一次会话（约 1 小时）**：B1 冒烟 + B2 综合采集（含 T5 FPS 两轮）+ B3 顺带执行。
3. **B 数据到达后**：闭合 A1 启用边界 → B4 验收；A2 完成后 B5；P6 出口前 B6。
4. 每个 B 项完成后，由分析侧产出对应报告并更新本表与 PLAN.md §1。

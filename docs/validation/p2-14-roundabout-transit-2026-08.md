...
### P2-19 审查追加（2026-08-10）
- 审查中发现 token `r_1w_*` 系列可能为环岛 prefab 命名（此前结论"token 无 roundabout 命名"
  基于 roundabout/_rb/kreis 关键词检索）——**r_1w 系列的拓扑判定未定案**：登记为待验证
  （若 r_1w 确为环岛，则"SCS 环岛放射直通结构"结论需复核——检测器判据可能过严）
- 环岛出口引导 maneuver 集成（"第 N 个出口驶出"）待几何识别落地（P2-19 已知限制登记）

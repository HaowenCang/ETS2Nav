...
> **后续推进（2026-08-12 ~ 2026-09-11）**：本节记录的是 P5 修复收尾当时的推送状态。此后 `main` 依次推进 `92cff89`（本记录 + tag/提交号回写）→ `0928e47`（ADR-008）→ `4b5e945` → `00f9a54`（离线加固四项）→ `19eceb1`（数据集 Release 分发）；并新增数据发布 tag `dataset-europe-v5`。截至 2026-09-11，`main` = `19eceb1`，与 `origin/main` 一致。最新发布后复核与增量门验证见 `p4-p6-hardening-2026-08.md` §9。

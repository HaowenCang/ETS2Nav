...
6. **§57 自动恢复缺失 + 速度项反相**（同前 MINOR）：手动拖动后自动恢复未实现；速度项的单调性与设计意图需复核。
7. **移动端 WS 默认地址 `127.0.0.1`**：真机需手动改为 PC 内网 IP（二维码已含 IP/端口）。
8. **UI 未按事件类型分发**：`map_state` 帧由 try/catch 静默吞掉（`933c187` 已部分覆盖），视觉确认登记 B5。

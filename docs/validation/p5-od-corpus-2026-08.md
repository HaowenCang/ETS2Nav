...
> **状态更新（2026-08-12）**：本段第 1~2 项已于 2026-08-12 完成根因排查与处置——
> UK 孤立与 ferry 悬空确认为**真实编译器缺陷并已修复**（transit 端点孤立 258→0、
> 主分量 75.78%→79.35%）；残余断簇经五路独立检验（含 TruckLib 数据级 oracle 零差异）
> 判定为**源数据拓扑**；uturn 在修复后数据集上不再检出。详见
> `p5-graph-defects-2026-08.md`（根因与排除检验）、`p5-graph-defects-verification-2026-08.md`
> （验证记录）、`p5-closeout-2026-08.md`（阶段关门）。**本段以下内容保留为修复前的历史记录。**

...
| 字段 | 语义 | 证据 |
|---|---|---|
| `interval[]` | `(green, yellow, red, yellow)` 各段秒数；**每个 unit 至多 1 条**（实测 1.60 归档：107 个含、171 个继承父级——勘误 2026-08-10，原记"cr_1x1 有 4 条"实为 cycle 4 条） | 官方注释 `# green > yellow > red > yellow`，如 `(15.0, 2.0, 23.0, 2.0)` |
...
| `sleep_time_start/end` | 夜间闪烁窗口（自午夜分钟） | 实测 1.60 主文件 23/278 个含（含变体文件 32）——勘误 2026-08-10，原记 46 为 1.55 时代值；官方注释 `1410 # 23:30`、`180 # 03:00`。**不是所有 profile 都有** → 必须逐 profile 解析（v0.2 §34） |
